...
def cut_wavs(experimental_data: str, wav_directory: str, output_directory: str):
    ...
    for index, row in experimental_data_table.iterrows():
        ...
        phone_offset = row['offset'] - row['previous_phone_onset']
...
def cut_middle_5_frames(input_directory: str, output_directory: str, dataset: str, feature_or_model: str, log_files_without_enough_frames: str):
    # dataset = test or reference

    # Define constants
    if feature_or_model == 'mfcc':
        SAMPLE_RATE = 100
        FRAME_NUM = 5
    elif feature_or_model == 'ds' or feature_or_model == 'deepspeech' or feature_or_model == 'wav2vec':
        SAMPLE_RATE = 50
        FRAME_NUM = 3
    ...
    with open(log_files_without_enough_frames, 'a', newline='') as tsv_file:
        ...
        for filename in os.listdir(input_directory):
            ...
            if os.path.isfile(file_path) and filename.endswith('.npy'):
                ...
                if dataset == 'reference':
                    # Split filename and extract start and end times
                    parts = filename.split('_')
                    start_time = float(parts[4])  # 5th field (0-indexed is 4)
                    end_time = float(parts[5].removesuffix('.npy'))   # 6th field (0-indexed is 5)
                    ...
                    middle_frame = math.floor(array.shape[0]/2)


                # Extract the middle n frames (ensure indices are within bounds)
                start_extract = middle_frame - math.floor(FRAME_NUM/2)
                end_extract = middle_frame + math.ceil(FRAME_NUM/2)

                if (start_extract < 0 or end_extract > array.shape[0]):
                    print(filename, "could not be cut to only the vowel, since the file only contains", array.shape[0], "frames, but the middle of the vowel is at", middle_frame)
                    tsv_writer.writerow([filename, end_frame - start_frame])
                else:
                    # Extract the frames
                    middle_frames = array[start_extract:end_extract]

                    # Save the new array
                    new_filename = f"vowel_only_{filename}"
                    new_file_path = os.path.join(output_directory, new_filename)
                    np.save(new_file_path, middle_frames)
                
                    count += 1
                    if count % 100 == 0:
                        print(count)

        print("processed and saved", count, "files")
