...
import numpy as np
import torch
from model_analysis.deepspeech.config.inference_config import TranscribeConfig
from model_analysis.deepspeech.model.data_loader import SpectrogramParser
from model_analysis.deepspeech.model.model_for_extraction import DeepSpeech
...
def load_model(device,
               model_path):
    model = DeepSpeech.load_from_checkpoint(checkpoint_path = hydra.utils.to_absolute_path(model_path), map_location=device, strict = False)
    model.eval()
...
def save_outputs(outputs, layer, i, destination_path):
    ...
    save_path = os.path.join(full_destination_path, save_filename)
...
def extract(cfg: TranscribeConfig,
    ...
    abs_audio_path = hydra.utils.to_absolute_path(cfg.audio_path)

    count = 0
    if list_file == '':
        for file in itertools.chain(
            glob.iglob('**/*.wav', root_dir=abs_audio_path, recursive=True),
            glob.iglob('**/*.aif', root_dir=abs_audio_path, recursive=True),
        ):
            if file.endswith('.aif'):
                file_no_extension = file.removesuffix('.aif')
            else:
                file_no_extension = file.removesuffix('.wav')

            if not file.endswith(('.wav', '.aif')):
                continue

            file_all = os.path.join(abs_audio_path, file)
        ...
        for line in f:
            ...
            # dataset_name = 'dataset'
            # destination_path = '/gpfsscratch/rech/tub/uzz69cv/transfo_multi/deepspeech_english'
            save_outputs(new_outputs, layer=layer, i=fili.removesuffix(".wav"), destination_path=destination_path)
            count += 1
            if count % 100 == 0:
                print(count)
    print('Done!')
